...
> R: Reinicia la cantidad del carrito a 0. Sino se reinicia el carrito, este va a tener un valor de memoria basura.
...
> R: El test falla. Esperaba 700, obtuvo 350
...
> R: Línea 24: `total += c->items[i].precio;`, suma el precio pero no multiplica por la cantidad.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Deberia multiplicar el precio por la cantidad: `total += c->items[i].precio * c->items[i].cantidad;`
...
> R: El test falla. Esperaba 0, pero recibio 1. Termino en segmentation fault.
...
> R: El operador incorrecto es <=, deberia ser < (estrictamente). Entonces, cuando la cantidad es 4, 4 < 4 da falso y rechaza.
...
> R: Si, las lineas 29 y 30 de la funcion carrito_descuento aparecen con ####, porque make cobertura corre test_unitarios.c. Esta funcion solo se prueba en test_integracion.c, entonces queda afuera.
...
> R: La diferencia es que un test unitario prueba una unica funcion. En cambio, un test de integracion prueba varias funciones que trabajan juntas. El test que detectó el bug fue el test unitario, ya que test_total_con_cantidad pertenece a él.
...
> R: Porque en C los arrays no tienen un control automatico de limites. C no valida, escribe en la memoria aunque no sea suya. En cambio en Python o Java, el array se verifica en tiempo de ejecucion, si se pide una posicion que no existe, salta el error. No te deja escribir en memoria que no es tuya.
...
> R: Escribir tests me obliga a pensar lo que se espera como resultado y despues comparar con lo que devuelve el codigo. En cambio, leyendo el codigo no simepre vemos que esta mal, un error pequeño puede pasar desapercibido porque el codigo se ve bien.
...
> R: Porque el bug sumaba precio sin multiplicar por cantidad. Si la cantidad es 1, precio × 1 da el mismo resultado que sumar solo precio, entonces el test pasa aunque el código esté mal. Solo con ese caso no podemos notar que el codigo está mal.
